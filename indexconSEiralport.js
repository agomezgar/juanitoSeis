   ...
   win.webContents.on('did-finish-load', () => {
    ...
    .then((ports) => {
      ...
      ports.forEach((port) => {
        ...
        listaPuertos.push(port.path);
      });
      enviaPuertos(listaPuertos);
    })
    .catch((err) => {
      console.error('Error al obtener la lista de puertos:', err);
    });
   });
  };
...
function enviaPuertos(listaPuertos){
  console.log("enviando");
 win.webContents.send("tomaPuertos",(listaPuertos));
}
ipcMain.on("conecta",function(event,puertoElegido){
  console.log("conectando a "+puertoElegido)
//var board = new five.Board();
const board = new five.Board({
  io: new five.Board.Options().io({
    port: new SerialPort(puertoElegido, { baudRate: 9600 }), // Reemplaza 'COM3' con el puerto adecuado.
  }),
});
